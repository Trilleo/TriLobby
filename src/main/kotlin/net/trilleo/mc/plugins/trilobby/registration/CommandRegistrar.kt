 ...
 *   the `/trilobby` parent command (alias `/tl`). A command with
 *   `name = "reload"` becomes `/trilobby reload`.
 * * **Main commands** (`isMainCommand = true`) – registered directly on the
 *   server [CommandMap] as standalone top-level commands.
 *
 * The `/trilobby` command provides tab-completion for all registered
 * sub-commands automatically.
 */
object CommandRegistrar {

    private const val COMMANDS_PACKAGE = "net.trilleo.mc.plugins.trilobby.commands"
    private const val ROOT_COMMAND = "trilobby"
     ...
     * @property isSubCommand `true` when the command is a sub-command of `/trilobby`
     */
     ...
     * and registers it either as a sub-command of `/trilobby` or as a
