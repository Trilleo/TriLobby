 ...
 * `net.trilleo.mc.plugins.trilobby.commands` package (or any subpackage) to
 * have it automatically discovered and registered at startup.
 *
 * By default every command is registered as a **sub-command** of `/trilobby`
 * (alias `/tl`). For example, a command with `name = "reload"` becomes
 * `/trilobby reload`. Set [isMainCommand] to `true` to register the command
 ...
 *
 * Example (sub-command – registered as `/trilobby start`):
 * ```kotlin
 ...
 *     usage = "/trilobby start",
 *     permission = "trilobby.start"
 * ) {
